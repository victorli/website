...
if(!function_exists('breadcrumbs_plus')) {
	...
	function breadcrumbs_plus( $args = '' ) {
		...
		$defaults = array(
			...
			'suffix' => '</p>',
			'title' => esc_html__( 'You are here: ', 'wt_front' ),
		...
		$home = '<a href="'. esc_url( home_url( '/' ) ) .'" rel="home" class="breadcrumbs-begin">' . $args['home'] . '</a>';
		...
		elseif ( is_search() )
			$html .= $home . $separator . _bold_( esc_html__( 'Search results for "', 'breadcrumbs-plus' ) . stripslashes( strip_tags( get_search_query() ) ) . '"', $args['bold'] );
		...
		elseif ( is_archive() ) {
			...
			elseif ( is_date() ) {
				...
				if ( is_day() )
					$html .= _bold_( esc_html__( 'Archives for ', 'breadcrumbs-plus' ) . get_the_time( 'F j, Y' ), $args['bold'] );
				
				elseif ( is_month() )
					$html .= _bold_( esc_html__( 'Archives for ', 'breadcrumbs-plus' ) . single_month_title( ' ', false ), $args['bold'] );
						
				elseif ( is_year() )
					$html .= _bold_( esc_html__( 'Archives for ', 'breadcrumbs-plus' ) . get_the_time( 'Y' ), $args['bold'] );
			}
	
			elseif ( is_author() )
				$html .= _bold_( esc_html__( 'Archives by: ', 'breadcrumbs-plus' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author ), $args['bold'] );
		}
		...
		elseif ( is_404() )
			$html .= $home . $separator . _bold_( esc_html__( 'Page Not Found', 'breadcrumbs-plus' ), $args['bold'] );
	
		$breadcrumbs = '<div id="wt_breadcrumbs">';

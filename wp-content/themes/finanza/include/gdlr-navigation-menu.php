	...
	if( !class_exists('gdlr_edit_nav_menu_walker') ){
		...
		class gdlr_edit_nav_menu_walker extends Walker_Nav_Menu{
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				?>
				<li id="menu-item-<?php echo gdlr_escape_content($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
					<dl class="menu-item-bar">
						<dt class="menu-item-handle">
							<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo gdlr_escape_content($submenu_text); ?>><?php _e( 'sub item', 'gdlr_translate' ); ?></span></span>
							<span class="item-controls">
								...
								<a class="item-edit" id="edit-<?php echo gdlr_escape_content($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'gdlr_translate'); ?>" href="<?php
					...
					<div class="menu-item-settings" id="menu-item-settings-<?php echo gdlr_escape_content($item_id); ?>">
						<?php if( 'custom' == $item->type ) : ?>
							<p class="field-url description description-wide">
								<label for="edit-menu-item-url-<?php echo gdlr_escape_content($item_id); ?>">
									<?php _e( 'URL', 'gdlr_translate' ); ?><br />
									<input type="text" id="edit-menu-item-url-<?php echo gdlr_escape_content($item_id); ?>" class="widefat code edit-menu-item-url" name="menu-item-url[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->url ); ?>" />
								</label>
							</p>
						<?php endif; ?>
						<p class="description description-thin">
							<label for="edit-menu-item-title-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Navigation Label', 'gdlr_translate' ); ?><br />
								<input type="text" id="edit-menu-item-title-<?php echo gdlr_escape_content($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
							</label>
						</p>
						<p class="description description-thin">
							<label for="edit-menu-item-attr-title-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Title Attribute', 'gdlr_translate' ); ?><br />
								<input type="text" id="edit-menu-item-attr-title-<?php echo gdlr_escape_content($item_id); ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
							</label>
						</p>
						<!-- new fields insertion starts here -->
						<p class="field-menu-icon description-wide">
							<label for="edit-gdlr-menu-icon-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Menu Icon', 'gdlr_translate' ); ?><br />
								<input type="text" id="edit-gdlr-menu-icon-<?php echo gdlr_escape_content($item_id); ?>" class="widefat edit-menu-item-attr-title" name="gdlr-menu-icon[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->gdlr_menu_icon ); ?>" />
							</label>
						</p>						
						<p class="field-menu-mega description">
							<label for="edit-gdlr-menu-item-mega-<?php echo gdlr_escape_content($item_id); ?>">
								<input type="checkbox" id="edit-gdlr-menu-item-mega-<?php echo gdlr_escape_content($item_id); ?>" value="mega_menu" name="gdlr-menu-item-mega[<?php echo gdlr_escape_content($item_id); ?>]"<?php checked($item->gdlr_mega_menu, 'mega_menu'); ?> />
								<?php _e( 'Enable Mega Menu', 'gdlr_translate' ); ?>
							</label>
						</p>		
						<p class="field-menu-section description">
							<label for="edit-gdlr-menu-item-section-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Section Size ( *Only when parent mega menu option is checked )', 'gdlr_translate' ); ?><br />
								<select id="edit-gdlr-menu-item-section-<?php echo gdlr_escape_content($item_id); ?>" name="gdlr-menu-item-section[<?php echo gdlr_escape_content($item_id); ?>]" >
						...
						<p class="field-link-target description">
							<label for="edit-menu-item-target-<?php echo gdlr_escape_content($item_id); ?>">
								<input type="checkbox" id="edit-menu-item-target-<?php echo gdlr_escape_content($item_id); ?>" value="_blank" name="menu-item-target[<?php echo gdlr_escape_content($item_id); ?>]"<?php checked( $item->target, '_blank' ); ?> />
								<?php _e( 'Open link in a new window/tab', 'gdlr_translate' ); ?>
							</label>
						</p>
						<p class="field-css-classes description description-thin">
							<label for="edit-menu-item-classes-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'CSS Classes (optional)', 'gdlr_translate' ); ?><br />
								<input type="text" id="edit-menu-item-classes-<?php echo gdlr_escape_content($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
							</label>
						</p>
						<p class="field-xfn description description-thin">
							<label for="edit-menu-item-xfn-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Link Relationship (XFN)', 'gdlr_translate' ); ?><br />
								<input type="text" id="edit-menu-item-xfn-<?php echo gdlr_escape_content($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
							</label>
						</p>
						<p class="field-description description description-wide">
							<label for="edit-menu-item-description-<?php echo gdlr_escape_content($item_id); ?>">
								<?php _e( 'Description', 'gdlr_translate' ); ?><br />
								<textarea id="edit-menu-item-description-<?php echo gdlr_escape_content($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo gdlr_escape_content($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						...
						<div class="menu-item-actions description-wide submitbox">
							...
							<?php endif; ?>
							<a class="item-delete submitdelete deletion" id="delete-<?php echo gdlr_escape_content($item_id); ?>" href="<?php
							echo wp_nonce_url(
								add_query_arg(
									array(
							...
							); ?>"><?php _e( 'Remove', 'gdlr_translate' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo gdlr_escape_content($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
								?>#menu-item-settings-<?php echo gdlr_escape_content($item_id); ?>"><?php _e('Cancel', 'gdlr_translate'); ?></a>
						</div>

						<input class="menu-item-data-db-id" type="hidden" name="menu-item-db-id[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo gdlr_escape_content($item_id); ?>" />
						<input class="menu-item-data-object-id" type="hidden" name="menu-item-object-id[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->object_id ); ?>" />
						<input class="menu-item-data-object" type="hidden" name="menu-item-object[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->object ); ?>" />
						<input class="menu-item-data-parent-id" type="hidden" name="menu-item-parent-id[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->menu_item_parent ); ?>" />
						<input class="menu-item-data-position" type="hidden" name="menu-item-position[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->menu_order ); ?>" />
						<input class="menu-item-data-type" type="hidden" name="menu-item-type[<?php echo gdlr_escape_content($item_id); ?>]" value="<?php echo esc_attr( $item->type ); ?>" />

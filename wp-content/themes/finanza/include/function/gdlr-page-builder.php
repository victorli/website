	...
	*/
	...
	if( !function_exists('gdlr_print_item') ){
		function gdlr_print_item( $content ){
			switch ($content['type']){
				case 'about-us': echo gdlr_get_about_us_item($content['option']); break;
				...
				case 'service-slide-show': echo gdlr_get_service_slideshow($content['option']); break;
				...
				case 'tab-slideshow': echo gdlr_get_tab_slideshow_item($content['option']); break;
	...
	if( !function_exists('gdlr_print_color_wrapper') ){
		function gdlr_print_color_wrapper( $content ){
			...
			if( !empty($content['option']['background']) || !empty($padding) ){
				echo 'style="';
				if( empty($content['option']['background-type']) || $content['option']['background-type'] == 'color' || $content['option']['background-type'] == 'color-2' ){
					echo !empty($content['option']['background'])? 'background-color: ' . esc_attr($content['option']['background']) . '; ': '';
				}
				echo gdlr_escape_content($border);
				echo gdlr_escape_content($padding);
				echo '" ';
			}
			echo '>';
			echo '<div class="container">';
			if( !empty($content['option']['background-type']) && $content['option']['background-type'] == 'color-2' && !empty($content['option']['background-left']) ){
				$column_class = gdlr_get_column_class(str_replace('-', '/',$content['option']['column-size']));
				$half_style  = 'background: ' . $content['option']['background-left'] . ';';
				$half_style .= 'top: -' . $content['option']['padding-top'] . ';';
				$half_style .= 'bottom: -' . $content['option']['padding-bottom'] . ';'; 
				
				echo '<div class="gdlr-half-color-wrapper-left ' . $column_class . '" >';
				echo '<div class="gdlr-half-color-wrapper-inner" style="' . $half_style . '" ></div>';
				echo '</div>';
			}
			$size = 0; 
		...
		function gdlr_print_parallax_wrapper( $content ){
			...
			if( !empty($content['option']['background']) && $content['option']['type'] == 'image' ){
				...
				if( !empty($content['option']['background-mobile']) ){
					...
					echo '<style type="text/css" scoped >@media only screen and (max-width: 767px){ ';

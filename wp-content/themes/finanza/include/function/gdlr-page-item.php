	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			$item_class  = (!empty($atts['carousel']) && $atts['title-type'] == 'left')? ' gdlr-nav-container': '';
			$item_class .= ' gdlr-' . $atts['title-type'];
			$item_class .= ' gdlr-' . $atts['title-size'];
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' && (!empty($atts['title']) || !empty($atts['caption'])) ){
				...
				if( !empty($atts['carousel']) && $atts['title-type'] == 'left' ){
					$ret .= gdlr_get_item_title_nav( $atts['carousel'] );
				}
				$ret .= '</div>';
			...
			if( !empty($atts['carousel']) && $atts['title-type'] == 'center' ){
				$ret .= '<div class="gdlr-item-title-nav gdlr-nav-container ' . $item_class . '">';
				$ret .= gdlr_get_item_title_nav( $atts['carousel'] );
				$ret .= '</div>';
			}
	...
	if( !function_exists('gdlr_get_item_title_nav') ){
		function gdlr_get_item_title_nav( $carousel ){
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			if( $settings['type'] == 'image' && !empty($settings['image']) ){
				$ret .= '<div class="column-service-image" >' . gdlr_get_image($settings['image']) . '</div>';
	...
	// service slideshow
	if( !function_exists('gdlr_get_service_slideshow') ){
		function gdlr_get_service_slideshow( $settings ){
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			
			$count = 1;
			if( empty($settings['columns']) || $settings['columns'] == 3 ){
				$settings['columns'] = 3;
				$column_class = array(
					'left'=>'eight columns',
					'right'=>'four columns',
			...
			$ret  = '<div class="gdlr-service-slideshow" ' . $item_id . '>';
			$ret .= '<div class="gdlr-service-slideshow-content gdlr-show-in-mobile"></div>';
			
			$ret .= '<div class="gdlr-service-slideshow-left ' . $column_class['left'] . '" >';
			$ret .= '<div class="gdlr-service-slideshow-dummy-height" ></div>';
			$ret .= '<div class="gdlr-service-slideshow-slides" >';
			for( $i=1; $i<=$settings['columns']; $i++ ){
				$image_src = wp_get_attachment_image_src($settings['image-' . $i], 'full');	
				$ret .= '<div class="service-slideshow-slide " data-id="' . $i . '" ';
				$ret .= 'style="background-image: url(' . $image_src[0] . ');" ></div>';
			}
			$ret .= '</div>'; // slides
			
			while( $count < $settings['columns'] ){
				$ret .= '<div class="' . $column_class['inner'] . '" >';
				$ret .= '<div class="gdlr-service-slideshow-item gdlr-item ' . ($count == 1? 'gdlr-active': '') . '" data-id="' . $count . '" >';
				$ret .= gdlr_get_image($settings['image-inner-' . $count], $settings['thumbnail-size']);
				$ret .= '<div class="service-slideshow-overlay" ></div>';
				$ret .= '<div class="service-slideshow-item-content gdlr-absolute-vcenter" >';
				$ret .= '<span class="service-slideshow-item-count">0' . $count . '</span>';
				$ret .= '<h3 class="service-slideshow-item-title">' . gdlr_text_filter($settings['title-' . $count]) . '</h3>';
				$ret .= '</div>';
				$ret .= '<div class="service-slideshow-hidden-content">';
				$ret .= '<div class="service-slideshow-gimmick"></div>';
				$ret .= '<div class="service-slideshow-title">' . gdlr_text_filter($settings['title-' . $count]) . '</div>';
				$ret .= '<div class="service-slideshow-content">' . gdlr_content_filter($settings['caption-' . $count]) . '</div>';
				if( !empty($settings['learn-more-link-' . $count]) ){
					$ret .= '<a class="service-slideshow-learn-more" href="' . $settings['learn-more-link-' . $count] . '" >' . $settings['learn-more-text-' . $count] . '</a>';
				}
				$ret .= '</div>';
				$ret .= '</div>'; // gdlr-item
			...
			$ret .= '<div class="gdlr-service-slideshow-right ' . $column_class['right'] . '" >';
			$ret .= '<div class="gdlr-service-slideshow-content gdlr-hide-in-mobile"></div>';
			
			$ret .= '<div class="gdlr-service-slideshow-item gdlr-item" data-id="' . $count . '" >';
			$ret .= gdlr_get_image($settings['image-inner-' . $count], $settings['thumbnail-size']);
			$ret .= '<div class="service-slideshow-overlay" ></div>';
			$ret .= '<div class="service-slideshow-item-content gdlr-absolute-vcenter" >';
			$ret .= '<span class="service-slideshow-item-count">0' . $count . '</span>';
			...
			$ret .= '</div>'; // service-slideshow-right
			$ret .= '</div>'; // gdlr-service-slideshow
			return $ret;
		}
	}	
	...
	// content item
	if( !function_exists('gdlr_get_about_us_item') ){
		function gdlr_get_about_us_item( $settings ){
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-item gdlr-about-us-item" ' . $item_id . $margin_style . '>';
			if( !empty($settings['title']) ){
				$ret .= '<h3 class="about-us-title">' . gdlr_text_filter($settings['title']) . '</h3>';
			}
			if( !empty($settings['caption']) ){
				$ret .= '<div class="about-us-caption">' . gdlr_text_filter($settings['caption']) . '</div>';
			}
			if( !empty($settings['title']) || !empty($settings['caption']) ){
				$ret .= '<div class="about-us-divider"></div>';
			}
			$ret .= '<div class="about-us-content">';
			$ret .= gdlr_content_filter($settings['content']);
			$ret .= '</div>';
			$ret .= '</div>';
			return $ret;
	...
	if( !function_exists('gdlr_get_skill_item') ){
		function gdlr_get_skill_item( $settings ){	
			...
			if( empty($settings['style']) || $settings['style'] == 'normal' ){
				...
				$ret .= '<div class="gdlr-skill-item-title gdlr-skin-title">' . $settings['title'] . '</div>';
				$ret .= '<div class="gdlr-skill-item-dot" ></div>';
				$ret .= '<div class="gdlr-skill-item-caption gdlr-skin-info">' . $settings['caption'] . '</div>';
				$ret .= '</div>'; // gdlr-skill-item-wrapper		
			}else{
				$text_color = 'style="color: ' . $settings['text-color'] . ';"';

				$margin .= 'background-color: ' . $settings['background'] . ';';
				$margin_style = (!empty($margin))? ' style="' . $margin . '" ': '';	

				$ret  = '<div class="gdlr-skill-item-wrapper gdlr-type-2 gdlr-item" ' . $item_id . $margin_style . '>';
				$ret .= '<div class="gdlr-skill-item-title gdlr-skin-title" ' . $text_color . '>' . $settings['title'] . '</div>';
				$ret .= '<div class="gdlr-skill-item-dot" style="border-color: ' . $settings['text-color'] . ';" ></div>';
				$ret .= '<div class="gdlr-skill-item-caption gdlr-skin-info" ' . $text_color . '>' . $settings['caption'] . '</div>';
				$ret .= '</div>'; // gdlr-skill-item-wrapper		
	...
	if( !function_exists('gdlr_get_tab_item') ){
		function gdlr_get_tab_item( $settings ){
			...
			if( !empty($settings['title']) && !empty($settings['caption']) ){
				$settings['title-size'] = 'medium';
				$settings['title-type'] = 'left';
				$ret .= gdlr_get_item_title($settings);
			}
			foreach( $tabs as $tab ){  $current_tab++;
				$ret .= '<h4 class="tab-title gdlr-content-font';
	...
	// tab item
	if( !function_exists('gdlr_get_tab_slideshow_item') ){
		function gdlr_get_tab_slideshow_item( $settings ){	
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-item gdlr-tab-slideshow-item">';
			$ret .= gdlr_get_item_title($settings);	
			$ret .= '<div class="gdlr-tab-item vertical" ' . $item_id . $margin_style . '>';
			$ret .= '<div class="tab-title-wrapper" >';
			foreach( $tabs as $tab ){  $current_tab++;
				$ret .= '<h4 class="tab-title gdlr-content-font';
				$ret .= ($current_tab == intval($settings['initial-state']))? ' active" >': '" >';			
				$ret .= '<span>' . gdlr_text_filter($tab['title']) . '</span></h4>';				
			}
			$ret .= '</div>'; // tab-title-wrapper
			
			// for slides
			$current_tab = 0;
			$ret .= '<div class="gdlr-tab-slides" >';
			foreach( $tabs as $tab ){  $current_tab++;
				$image_src = wp_get_attachment_image_src($tab['background'], 'full');	
				$ret .= '<div class="gdlr-tab-slide " data-id="' . $current_tab . '" ';
				$ret .= 'style="background-image: url(' . $image_src[0] . '); ' . ($current_tab == intval($settings['initial-state'])? 'display:block;':'') . '" ></div>';	
			}
			$ret .= '</div>'; // gdlr-tab-slides
			...
			foreach( $tabs as $tab ){  $current_tab++;
				$ret .= '<div class="tab-content';
				$ret .= ($current_tab == intval($settings['initial-state']))? ' active" >': '" >';
				$ret .= gdlr_content_filter($tab['caption']) . '</div>';
							
			}	
			$ret .= '</div>'; // tab-content-wrapper

			$ret .= '<div class="clear"></div>';
			$ret .= '</div>'; // gdlr-tab-item 
			$ret .= '</div>'; // gdlr-tab-slideshow-item 
	...
	if( !function_exists('gdlr_get_stunning_text_item') ){
		function gdlr_get_stunning_text_item( $settings ){
			...
			$ret  = '<div class="gdlr-item gdlr-stunning-item" ' . $item_id . $margin_style . '>';
			$ret .= '<h2 class="stunning-item-title">' . gdlr_text_filter($settings['title']) . '</h2>';
			$ret .= '<div class="stunning-item-caption gdlr-skin-content">' . gdlr_content_filter($settings['caption']) . '</div>';
			$ret .= '</div>'; // gdlr-item
	...
	if( !function_exists('gdlr_get_carousel_testimonial_item') ){
		function gdlr_get_carousel_testimonial_item( $settings ){	
			...
			$settings['carousel'] = '<span class="gdlr-testimonial-carousel-icon">“</span>';
			$ret .= gdlr_get_item_title($settings);							
	...
	if( !function_exists('gdlr_get_static_personnel_item') ){
		function gdlr_get_static_personnel_item( $settings ){	
			...
			$settings['text-align'] = empty($settings['text-align'])? 'gdlr-center': 'gdlr-' . $settings['text-align'];
			...
			foreach( $list as $tab ){ 
				...
				$ret .= '<div class="' . gdlr_get_column_class('1/' . $item_size) . '">';
				$ret .= '<div class="gdlr-item gdlr-personnel-item ' . $settings['personnel-style'] . ' ' . $settings['text-align'] . '">';
